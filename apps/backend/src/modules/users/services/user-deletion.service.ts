  ...
  async processScheduledDeletions(): Promise<number> {
    ...
    for (const user of usersToDelete) {
      try {
        ...
        this.logger.error('Erro ao deletar dados de usuário durante limpeza automática', error instanceof Error ? error.stack : error);
  ...
  async sendDeletionReminders(): Promise<number> {
    ...
    for (const user of usersToRemind) {
      try {
        ...
        // SEGURANÇA: Não loga userId ou email para evitar exposição de dados pessoais em logs
        this.logger.error('Erro ao enviar lembrete de deleção de conta', error instanceof Error ? error.stack : error);
      }
    }
