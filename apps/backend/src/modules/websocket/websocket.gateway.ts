  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      this.logger.error(`Erro na autenticação do cliente ${client.id}:`, error instanceof Error ? error.message : 'Unknown error');
      client.disconnect();
    }
  }
  ...
  sendNotificationToUser(userId: string, notification: any) {
    ...
    this.logger.debug(`Notificação enviada para ${socketIds.size} conexão(ões) do usuário ${userId}`);
    return true;
  }
