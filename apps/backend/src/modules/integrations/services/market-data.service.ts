...
export class MarketDataService {
   ...
   */
  async getCryptoData(ticker: string): Promise<{ currentPrice: number; priceChange24h: number; priceChangePercent24h: number; symbol?: string }> {
    try {
   ...
   */
  async getStockData(ticker: string): Promise<{ price: number; change: number; changePercent: number; currency?: string }> {
    try {
      const symbol = this.normalizeTicker(ticker);
      const quote = (await yahooFinance.quote(symbol)) as any;
      ...
      const currency = quote.currency || quote.financialCurrency || (quote.price && quote.price.currency) || 'USD';
