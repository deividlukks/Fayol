...
describe('TwoFactorService', () => {
  ...
  describe('setup2FA', () => {
    ...
    it('deve lançar BadRequestException se 2FA já está ativado', async () => {
      ...
      await expect(service.setup2FA(mockUser.id, 'password')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('verifySetup', () => {
    ...
    it('deve lançar BadRequestException se 2FA não foi iniciado', async () => {
      ...
      await expect(service.verifySetup(mockUser.id, '123456')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('verifyCode', () => {
    ...
    it('deve lançar BadRequestException se 2FA não está ativado', async () => {
      ...
      await expect(service.verifyCode(mockUser.id, '123456')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('verifyBackupCode', () => {
    it('deve retornar true e remover backup code usado', async () => {
      ...
      mockPrismaService.user.findUnique.mockResolvedValue(userWith2FA);
      jest.spyOn(bcrypt, 'compare').mockResolvedValueOnce(false as never).mockResolvedValueOnce(true as never);
      mockPrismaService.user.update.mockResolvedValue({
        ...userWith2FA,
  ...
  describe('disable2FA', () => {
    ...
    it('deve lançar BadRequestException com código inválido', async () => {
      ...
      await expect(
        service.disable2FA(mockUser.id, 'password', 'invalid-code'),
  ...
  describe('get2FAStatus', () => {
    ...
    it('deve lançar BadRequestException se usuário não existe', async () => {
      ...
      await expect(service.get2FAStatus('invalid-id')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
