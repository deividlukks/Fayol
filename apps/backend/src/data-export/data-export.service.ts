...
export class DataExportService {
  ...
  async createExportRequest(
    ...
    this.processExport(exportRequest.id).catch((error) => {
      this.logger.error(`Erro ao processar exportação ${exportRequest.id}`, error instanceof Error ? error.stack : error);
    });
  ...
  private convertToCSV(data: UserDataExport): string {
    ...
    lines.push(
      `${data.user.id},${data.user.name},${data.user.email},${data.user.createdAt}`,
    );
    lines.push('');
    ...
    for (const request of expiredRequests) {
      if (request.downloadUrl) {
        ...
        try {
          ...
          this.logger.error('Falha ao deletar arquivo de exportação', error instanceof Error ? error.stack : error);
        }
      }
