...
describe('Dashboard (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Criar algumas transações para testes
    ...
    // Criar receita
    const incomeCategory = categoriesResponse.body.find((c: any) => c.type === 'income')?.id;
    await request(app.getHttpServer())
      .post('/api/v1/transactions')
      .set('Authorization', `Bearer ${accessToken}`)
      .send({
    ...
    it('deve retornar score de saúde financeira', () => {
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          // Level deve ser um dos valores válidos
  ...
  describe('Isolamento de Dados', () => {
    ...
    beforeAll(async () => {
      ...
      // Criar conta e transação para o outro usuário
      ...
      const otherCategory = categoriesResponse.body.find((c: any) => c.type === 'expense')?.id;
