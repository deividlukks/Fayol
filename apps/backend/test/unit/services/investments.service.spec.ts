...
describe('InvestmentsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException when account not found', async () => {
      ...
      await expect(service.create(mockUser.id, createDto)).rejects.toThrow('Conta de custódia não encontrada.');
    });
    ...
    it('should calculate total cost correctly and debit account', async () => {
      ...
      const expectedCost = 100 * 30.50; // 3050
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException when investment not found', async () => {
      ...
      await expect(service.findOne('nonexistent-id', mockUser.id)).rejects.toThrow('Investimento não encontrado.');
    });
    ...
    it('should throw ForbiddenException when investment belongs to different user', async () => {
      ...
      await expect(service.findOne('investment-123', mockUser.id)).rejects.toThrow(ForbiddenException);
      await expect(service.findOne('investment-123', mockUser.id)).rejects.toThrow('Acesso negado.');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when investment not found', async () => {
      ...
      await expect(service.update('nonexistent-id', mockUser.id, updateDto)).rejects.toThrow(NotFoundException);
