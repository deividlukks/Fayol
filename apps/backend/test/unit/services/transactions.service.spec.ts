...
describe('TransactionsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException when account not found', async () => {
      ...
      await expect(service.create(mockUser.id, createDto)).rejects.toThrow('Conta de origem não encontrada.');
    });
    ...
    it('should use AI to predict category when categoryId is not provided', async () => {
      ...
      expect(aiService.predictCategory).toHaveBeenCalledWith(mockUser.id, dtoWithoutCategory.description);
    });
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException when transaction not found', async () => {
      ...
      await expect(service.findOne('nonexistent-id', mockUser.id)).rejects.toThrow(NotFoundException);
      await expect(service.findOne('nonexistent-id', mockUser.id)).rejects.toThrow('Transação não encontrada.');
    });
    ...
    it('should throw ForbiddenException when transaction belongs to different user', async () => {
      ...
      await expect(service.findOne('transaction-123', mockUser.id)).rejects.toThrow(ForbiddenException);
      await expect(service.findOne('transaction-123', mockUser.id)).rejects.toThrow('Acesso negado.');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should trigger AI learning when category is changed', async () => {
      ...
      prismaService.transaction.findUnique.mockResolvedValue(mockTransaction);
      prismaService.category.findUnique.mockResolvedValue({ id: 'new-category-id', name: 'New Category' });
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException when trying to delete non-existent transaction', async () => {
      ...
      await expect(service.remove('nonexistent-id', mockUser.id)).rejects.toThrow(NotFoundException);
    });
  });
});
