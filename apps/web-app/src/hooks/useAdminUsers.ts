...
export function useAdminUsers(initialFilters: AdminUsersFilters = {}) {
  ...
  const restoreUser = async (id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao restaurar usuário';
      throw new Error(errorMessage);
    }
  };

  const updateRoles = async (id: string, roles: UserRole[]) => {
    try {
      await adminUsersService.updateRoles(id, roles);
      await fetchUsers();
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Erro ao atualizar roles';
      throw new Error(errorMessage);
    }
  };
