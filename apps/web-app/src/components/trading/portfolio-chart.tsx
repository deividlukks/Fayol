...
export function PortfolioChart() {
  ...
  // 1. Definição de Cores (Memoizado para evitar recriação e alertas de lint)
  const colors = useMemo(
    () => ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#ec4899', '#6366f1'],
    []
  );
  ...
  // 3. Cálculos e Hooks (SEMPRE antes dos returns condicionais)

  // Calcular totais
  const { totalCost, totalCurrent, totalPnl, totalPnlPercent } = useMemo(() => {
    const cost = portfolioData.reduce((sum, item) => sum + item.costTotal, 0);
    const current = portfolioData.reduce((sum, item) => sum + item.currentTotal, 0);
    const pnl = current - cost;
    const pnlPercent = cost > 0 ? (pnl / cost) * 100 : 0;
    return { totalCost: cost, totalCurrent: current, totalPnl: pnl, totalPnlPercent: pnlPercent };
  }, [portfolioData]);
  ...
  const pnlData: ChartData[] = useMemo(() => {
    return portfolioData.map((item) => ({
      symbol: item.symbol,
  ...
  // Componentes de Tooltip (definidos fora ou dentro, mas o Recharts aceita function components)
  function PieTooltip({ active, payload }: TooltipProps<number, string>) {
    ...
    return (
      <div className="bg-white border p-2 rounded shadow-sm text-sm">
        ...
        <div>{CurrencyUtils.format(item.value ?? 0)}</div>
        <div className="text-xs text-slate-500">
          Composição: {totalCurrent > 0 ? (((item.value ?? 0) / totalCurrent) * 100).toFixed(2) : 0}
          %
        </div>
      </div>
    );
