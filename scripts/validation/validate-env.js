...
function checkEnvVariables() {
  ...
  // Variáveis condicionalmente obrigatórias
  const conditionalVars = {
    TELEGRAM_BOT_TOKEN: {
      ...
      conditionMessage: 'Obrigatório quando FEATURE_TELEGRAM_BOT=true',
    },
  };
  ...
  const optionalVars = {
    ...
    DATABASE_POOL_MAX: {
      description: 'Máximo de conexões no pool (Prisma 7)',
      default: '10',
      validate: (value) => {
        const num = parseInt(value);
        return !isNaN(num) && num > 0 && num <= 100;
      },
      warningMessage: 'DATABASE_POOL_MAX deve ser um número entre 1 e 100',
    },
    DATABASE_IDLE_TIMEOUT: {
      description: 'Timeout para conexões inativas em ms (Prisma 7)',
      default: '10000',
      validate: (value) => {
        const num = parseInt(value);
        return !isNaN(num) && num >= 0;
      },
      warningMessage: 'DATABASE_IDLE_TIMEOUT deve ser um número >= 0',
    },
    DATABASE_CONNECTION_TIMEOUT: {
      description: 'Timeout para estabelecer conexão em ms (Prisma 7, 0 = sem timeout)',
      default: '0',
      validate: (value) => {
        const num = parseInt(value);
        return !isNaN(num) && num >= 0;
      },
      warningMessage: 'DATABASE_CONNECTION_TIMEOUT deve ser um número >= 0',
    },
    REDIS_HOST: {
  ...
  // Verificar variáveis condicionalmente obrigatórias
  console.log();
  log('Variáveis Condicionais:', 'blue');
